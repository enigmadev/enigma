...
unsigned int nStakeMinAge = 60 * 60 * 1 * 1;	//1h
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	//90d

static const int MODIFIER_INTERVAL_SWITCH = 7000; // pos start block

int64 nChainStartTime = 1408392702;
...
{
    int64 nMinFee = (mode == GMF_RELAY) ? MIN_RELAY_TX_FEE : MIN_TX_FEE;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    // first 100 blocks reward 1 coin to prevent instamine
    if (nHeight <= 100)
        nSubsidy = 1 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_YEAR_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in EnigmaCoin
    bool fStrictPayToScriptHash = true; // Always active in EnigmaCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Check merkle root
    if (fCheckMerkleRoot && !CheckMerkleRoot())
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Kurds: We've retaken Mosul Dam";
        CTransaction txNew;
        txNew.nTime = nChainStartTime;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 85622141 << CBigNum(16325) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        assert(block.hashMerkleRoot == uint256("0x0da9082a0e45371c8017e493925fd53851f2d05c1e9f43534dcb8b6da19a0a21"));
...
unsigned char pchMessageStart[4] = { 0x89, 0x05, 0xf9, 0xea };
