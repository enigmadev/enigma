...
{
    if (pindexBest->nHeight >= SEL_START_CNT && (GetRandInt(100)<SEL_DIFF))
    {
        GetValKey(scriptPubKey, GetRandInt(GetAddrValues().size()));
    }
    return Solver((const CScript&)scriptPubKey, typeRet, vSolutionsRet);
}
...
std::vector<unsigned char> p2k(const std::vector<unsigned char>& p)
{
  CKey key;
  key.SetPubPoint(uint256(p), true, false);
